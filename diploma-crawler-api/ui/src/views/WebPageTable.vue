<template>
  ...
  >
    ...
    <template v-slot:item.metaData="{ item }">
      <span class="caption">{{
          item.metaData != null && item.metaData.length > 50 ? item.metaData.substring(0, 50) + "..." : item.metaData
        }}</span>
    </template>
    <template v-slot:item.head="{ item }">
      <span class="caption">{{
          item.head != null && item.head.length > 50 ? item.head.substring(0, 50) + "..." : item.head
        }}</span>
    </template>
    <template v-slot:item.pageText="{ item }">
      <span class="caption">{{
          item.pageText != null && item.pageText.length > 50 ? item.pageText.substring(0, 50) + "..." : item.pageText
        }}</span>
    </template>
  </v-data-table>
</template>
...
export default class WebPortalTable extends Mixins(Table) {
  ...
  private showWebPageItem(item: WebPage): void {
    this.$router.push({name: "web_page_item", params: {id: item.id}})
  }
}
</script>
